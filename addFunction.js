function addElements(firstElement, secondElement) {
  ...
  if ((!firstElement || re.test(firstElement)) && 
    (!secondElement || re.test(secondElement)))
  {
...
function arrayToObject(arrayElements) {
    ...
    for (let item of arrayElements) {
      ...
      if (item.includes('x')) {
        ...
        objectElements = {
          ...objectElements,
          [phrases[1]]: parseFloat(phrases[0])
        };
      } else {
        objectElements = { 
          ...objectElements, 
...
function main(firstElement, secondElement) {
...
// Change these arguments to try another statement.
console.log(main('2*x^2 + 3', '3*x^3 + x^2'));
